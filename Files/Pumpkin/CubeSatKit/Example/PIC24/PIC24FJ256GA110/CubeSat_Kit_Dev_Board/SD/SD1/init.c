...
$Source: C:\\RCS\\D\\Pumpkin\\CubeSatKit\\Example\\PIC24\\PIC24FJ256GA110\\CubeSat_Kit_Dev_Board\\SD\\SD1\\init.c,v $
$Author: aek $
$Revision: 3.1 $
...
void init(void) {
  ...
  // In this application we talk to the user via the USB.
  // N.B. To talk over MHX, use cskmhx_open instead. If Digi XStream modems
  //  are used, HS3 (-RTS_MHX) must be enabled, and SLEEP (HS4) must be
  //  disabled.
  csk_usb_open();
...
Used to create Salvo system ticks.
...
Receive characters from CSK's Rx0.
...
Receive characters from CSK's Rx1.
